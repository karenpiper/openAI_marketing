...
export function architectureDiagram(
  ...
  const tint = (svg: string, id: string) =>
    !focused
      ? svg
      : `<g opacity="${active.has(id) ? 1 : 0.28}">${active.has(id) ? svg.replaceAll('fill="#fffdf7"', 'fill="#fff0c2"').replaceAll('stroke-width="2"', 'stroke-width="4"') : svg}</g>`;
  ...
  const arrow = (
    ...
    return `<g opacity="${opacity}"><path d="M ${x} ${y} L ${xx} ${yy}" fill="none" stroke="#365343" stroke-width="2"/><path d="M ${xx} ${yy} L ${xx - 11 * Math.cos(a - 0.45)} ${yy - 11 * Math.sin(a - 0.45)} L ${xx - 11 * Math.cos(a + 0.45)} ${yy - 11 * Math.sin(a + 0.45)} Z" fill="#365343"/></g>`.replaceAll(
      "#365343",
      color,
    );
  };
  ...
  const proposed = [...edges]
    ...
    .map((edge) => {
      ...
      const [x, y] = anchors[a],
        [xx, yy] = anchors[b];
      return `<path d="M ${x} ${y} Q ${Math.max(x, xx) + 25} ${(y + yy) / 2} ${xx} ${yy}" stroke="#a85c00" stroke-width="4" stroke-dasharray="9 6" fill="none"/><circle cx="${xx}" cy="${yy}" r="5" fill="#a85c00"/>`;
    })
    .join("");
 ...
 <rect x="910" y="40" width="60" height="795" fill="${focused && active.has("F") ? "#fff0c2" : "#fffdf7"}" stroke="#365343" stroke-width="${focused && active.has("F") ? 4 : 2}" opacity="${focused && !active.has("F") ? 0.28 : 1}"/>
